...
def check_params(params):
    ...
    params.tst_test_dataset = {
        lang: {
            label_pairs: (os.path.join(params.data_path, 'tst.%s.%s.%s.pth' % (lang, label_pairs, label_pairs[0])),
                   os.path.join(params.data_path, 'tst.%s.%s.%s.pth' % (lang, label_pairs, label_pairs[1])))
...
def get_transferred_sentence(len1, lang2_id, modified_enc, decoder, dico):
    max_len = int(1.5 * len1.max().item() + 10)
    generated, lengths = decoder.generate(modified_enc, len1, lang2_id, max_len=max_len)
        ...
        for label_pair in [(0, 1), (1, 0)]:
            ...
            for src_batch, tgt_batch in zip(src_iterator, tgt_iterator):
                ...
                while True:
                    ...
                    logger.info("Iteration %d, Pred: %f, Loss: %f" % (it, pred, loss.item()))
