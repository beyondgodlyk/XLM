...
class Evaluator(object):
    ...
    def get_iterator(self, data_set, lang1, lang2=None, stream=False):
        ...
        """
        assert data_set in ['valid', 'test'] if not self.params.domain_adaptive else ['valid', 'test', 'domain.valid', 'domain.test', 'domain_lowercased.valid', 'domain_lowercased.test']
    ...
    def create_reference_files(self):
        ...
        for (lang1, lang2), v in self.data['para'].items():
            ...
            for data_set in ['valid', 'test'] if not params.domain_adaptive else ['valid', 'test', 'domain.valid', 'domain.test', 'domain_lowercased.valid', 'domain_lowercased.test']:
    ...
    def mask_out(self, x, lengths, rng):
        ...
        for i in range(bs):
            ...
            if not np.any(to_predict[:lengths[i] - 1, i]):
                ...
                to_predict[v, i] = 1
        pred_mask = torch.from_numpy(to_predict.astype(np.bool_))
    ...
    def run_all_evals(self, trainer):
        ...
        with torch.no_grad():
            ...
            for data_set in ['valid', 'test'] if not params.domain_adaptive else ['valid', 'test', 'domain.valid', 'domain.test', 'domain_lowercased.valid', 'domain_lowercased.test']:
                ...
                for lang1, lang2 in set(params.mt_steps + [(l2, l3) for _, l2, l3 in params.bt_steps] + [(l1, l2) for l1, l2, _ in params.bt_steps if params.domain_adaptive]):
...
class EncDecEvaluator(Evaluator):
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        """
        params = self.params
        assert data_set in ['valid', 'test'] if not params.domain_adaptive else ['valid', 'test', 'domain.valid', 'domain.test', 'domain_lowercased.valid', 'domain_lowercased.test']
        assert lang1 in params.langs
