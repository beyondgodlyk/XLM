...
DOMAIN_MIXED_LOWERCASED_SRC_TOK=$MONO_PATH/$SRC/domain.mixed_lowercased.$SRC.tok
...
DOMAIN_MIXED_LOWERCASED_SRC_TRAIN_BPE=$PROC_PATH/domain.mixed_lowercased.train.$SRC
...
if ! [[ -f "$DOMAIN_FOURSQ_LOWERCASED_SRC_TOK" ]]; then
  echo "Converting Mixed, FourSquare and Yelp EN data to lowercase..."
  eval "cat $DOMAIN_MIXED_SRC_TOK | $LOWERCASE > $DOMAIN_MIXED_LOWERCASED_SRC_TOK"
...
if ! [[ -f "$DOMAIN_MIXED_SRC_TRAIN_BPE" ]]; then
  ...
  $FASTBPE applybpe $DOMAIN_MIXED_LOWERCASED_SRC_TRAIN_BPE $DOMAIN_MIXED_LOWERCASED_SRC_TOK $BPE_CODES
...
if ! [[ -f "$DOMAIN_MIXED_SRC_TRAIN_BPE.pth" ]]; then
  ...
  $MAIN_PATH/preprocess.py $FULL_VOCAB $DOMAIN_MIXED_LOWERCASED_SRC_TRAIN_BPE
...
echo "    $SRC: $DOMAIN_MIXED_LOWERCASED_SRC_TRAIN_BPE.pth"
